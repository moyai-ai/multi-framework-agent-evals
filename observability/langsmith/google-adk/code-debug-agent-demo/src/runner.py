...
class ScenarioRunner:
    ...
    def __init__(self, agent_name: Optional[str] = None, enable_tracing: bool = False):
        """Initialize the scenario runner.

        Args:
            agent_name: Optional specific agent to use
            enable_tracing: Enable LangSmith tracing for all executions
        """
        ...
        # Check if LangSmith tracing is requested and configured
        if self.enable_tracing:
            import os
            if not os.getenv("LANGSMITH_API_KEY"):
                logger.warning("LangSmith tracing requested but LANGSMITH_API_KEY not found!")
                logger.warning("Tracing will be disabled. Set LANGSMITH_API_KEY to enable.")
                self.enable_tracing = False
            else:
                logger.info("LangSmith tracing enabled for scenario execution")
                logger.info(f"Project: {os.getenv('LANGSMITH_PROJECT', 'default')}")
    ...
    async def run_scenario(self, scenario: Scenario) -> ScenarioReport:
        ...
        # If tracing is enabled, wrap execution with LangSmith tracing
        if self.enable_tracing:
            return await self._run_scenario_traced(scenario)
        else:
            return await self._run_scenario_untraced(scenario)

    async def _run_scenario_traced(self, scenario: Scenario) -> ScenarioReport:
        """Run a scenario with LangSmith tracing enabled.
        ...
        return await execute_traced()

    async def _run_scenario_untraced(self, scenario: Scenario) -> ScenarioReport:
        """Run a scenario without tracing (standard execution).
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == "--all-scenarios" or sys.argv[1] == "--all":
            # Check for --traced flag
            enable_tracing = "--traced" in sys.argv
            if enable_tracing:
                sys.argv.remove("--traced")
            ...
            for scenario_file in scenario_files:
                ...
                runner = ScenarioRunner(agent_name=agent_name, enable_tracing=enable_tracing)
                try:
            ...
            if all_reports:
                print("\n" + ScenarioRunner(agent_name=agent_name, enable_tracing=enable_tracing).generate_report(all_reports))

            # Give time for async cleanup
            await asyncio.sleep(0.2)
            return

        # Check for --traced flag in single file mode
        enable_tracing = "--traced" in sys.argv
        if enable_tracing:
            sys.argv.remove("--traced")
            print("\n🔍 LangSmith tracing enabled")
