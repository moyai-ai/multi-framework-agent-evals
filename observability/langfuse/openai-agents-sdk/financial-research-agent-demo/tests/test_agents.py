"""
Comprehensive test suite for the Financial Research Agent System with Langfuse Observability.
"""
...
class TestAgents:
    ...
    def test_list_agents(self):
        ...
        assert "Financials Analyst" in agent_names
    ...
    def test_agent_configurations(self):
        ...
        search_tool_name = search_agent.tools[0].name if hasattr(search_agent.tools[0], 'name') else str(search_agent.tools[0])
        assert "web_search" in search_tool_name.lower()

        # Financials analyst has tools
        assert len(financials_analyst_agent.tools) == 2
        tool_names = [tool.name if hasattr(tool, 'name') else str(tool) for tool in financials_analyst_agent.tools]
        tool_names_str = " ".join(tool_names).lower()
        assert "financials" in tool_names_str or "company_financials" in tool_names_str
        assert "market_data" in tool_names_str or "market" in tool_names_str

        # Risk analyst has risk_analysis_tool
        assert len(risk_analyst_agent.tools) == 1
        risk_tool_name = risk_analyst_agent.tools[0].name if hasattr(risk_analyst_agent.tools[0], 'name') else str(risk_analyst_agent.tools[0])
        assert "risk" in risk_tool_name.lower()

        # Writer has analyst tools
        assert len(writer_agent.tools) == 3
        writer_tool_names = [tool.name if hasattr(tool, 'name') else str(tool) for tool in writer_agent.tools]
        writer_tool_names_str = " ".join(writer_tool_names).lower()
        assert "financials" in writer_tool_names_str or "company_financials" in writer_tool_names_str
        assert "risk" in writer_tool_names_str
        assert "market" in writer_tool_names_str
    ...
    def test_agent_model_configuration(self):
        """Test that all agents have model configured."""
        for key, agent in AGENTS.items():
...
class TestTools:
    ...
    def test_generate_search_terms_ai_semiconductor(self):
        """Test search term generation for AI semiconductor query."""
        terms = generate_search_terms("AI semiconductor market analysis")
        assert len(terms) > 0
        assert len(terms) <= 4
        assert any("ai" in term.lower() or "semiconductor" in term.lower() for term in terms)
...
class TestScenarioRunner:
    ...
    def test_validate_results_verification_status(self, scenario_runner):
        """Test validation of verification status."""
        expectations = ScenarioExpectations(verification_should_pass=True)

        # Should pass when verification status matches
        errors = scenario_runner.validate_results(
            expectations,
            search_terms_count=3,
            report="Test report",
        ...
        # Should fail when verification status doesn't match
        errors = scenario_runner.validate_results(
            expectations,
            search_terms_count=3,
            report="Test report",
...
class TestLangfuseObservability:
    ...
    def test_manager_has_traceable_decorators(self):
        """Test that manager methods are decorated for tracing."""
        from src.manager import FinancialResearchManager
        import inspect
        ...
        assert hasattr(manager, '_plan_searches')
        assert callable(manager._plan_searches)
        assert hasattr(manager, '_perform_searches')
        assert callable(manager._perform_searches)
        assert hasattr(manager, '_write_report')
        assert callable(manager._write_report)
        assert hasattr(manager, '_verify_report')
        assert callable(manager._verify_report)

    def test_runner_has_traceable_decorators(self):
        """Test that runner methods are decorated for tracing."""
    ...
    def test_manager_initialization_with_user_id(self):
        """Test manager initialization with user_id for tracing."""
        from src.manager import FinancialResearchManager

        manager = FinancialResearchManager(verbose=False, user_id="test-user-123")
        assert manager.user_id == "test-user-123"

    def test_manager_initialization_default_user_id(self):
        """Test manager initialization with default user_id."""
        from src.manager import FinancialResearchManager

        manager = FinancialResearchManager(verbose=False)
        assert manager.user_id == "default-user"

    def test_langfuse_tracing_disabled_in_tests(self, disable_langfuse_tracing):
        """Test that Langfuse tracing is disabled for unit tests."""
        # Verify that Langfuse keys are not set
        assert os.environ.get("LANGFUSE_PUBLIC_KEY") is None
        assert os.environ.get("LANGFUSE_SECRET_KEY") is None
        assert os.environ.get("LANGFUSE_DISABLED") == "true"

    def test_langfuse_client_handles_missing_keys(self):
        """Test that Langfuse client handles missing API keys gracefully."""
        from langfuse import get_client
        
        # When keys are missing, get_client should still return a client
        # (it may be a no-op client, but it shouldn't crash)
        client = get_client()
