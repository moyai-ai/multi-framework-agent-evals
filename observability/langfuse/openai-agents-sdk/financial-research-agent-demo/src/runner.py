"""
Test runner for executing financial research scenarios with Langfuse tracing.

This module provides functionality to load and execute predefined
research scenarios for testing the multi-agent system with full observability.
"""
...
class ScenarioRunner:
    """
    Executes test scenarios with full Langfuse observability.
    """

    def __init__(self, verbose: bool = False):
        """
        Initialize the scenario runner with Langfuse.
    ...
    @observe(name="run_test_scenario")
    async def run_scenario(self, scenario: TestScenario) -> ScenarioReport:
        """
        Execute a complete test scenario with Langfuse tracing.
        ...
        langfuse.update_current_trace(
            ...
            tags=["test-scenario", "financial-research"],
            metadata={
                "scenario_name": scenario.name,
                "description": scenario.description,
                "expectations": asdict(scenario.expectations)
            }
        )
        ...
        # Run the research
        manager = FinancialResearchManager(verbose=self.verbose, user_id=f"test-{scenario.name}")
        try:
        ...
        # Generate report
        langfuse_url = f"https://cloud.langfuse.com/trace/{trace_id}" if trace_id else None
        ...
        langfuse.update_current_trace(
            output={
                "validation_passed": scenario_report.validation_passed,
                "validation_errors": validation_errors,
                "execution_time_ms": scenario_report.execution_time_ms
            }
        )
...
async def main():
    """
    Main entry point for running scenarios with Langfuse tracing.
    """
    import argparse

    parser = argparse.ArgumentParser(
        description='Run financial research test scenarios with Langfuse observability'
    )
    parser.add_argument(
        'scenario_file',
        nargs='?',
    ...
    if not os.environ.get('LANGFUSE_PUBLIC_KEY') or not os.environ.get('LANGFUSE_SECRET_KEY'):
        print("Warning: LANGFUSE_PUBLIC_KEY and/or LANGFUSE_SECRET_KEY not set")
        print("Tracing will not work without Langfuse credentials")
    ...
    if args.all_scenarios:
        ...
        for report in all_reports:
            if report.langfuse_url:
                print(f"  • {report.scenario_name}: {report.langfuse_url}")
        print()
