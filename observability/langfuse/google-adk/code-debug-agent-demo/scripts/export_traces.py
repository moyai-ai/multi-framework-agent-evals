...
Usage:
    ...
    uv run scripts/export_traces.py --name "Code Debug" --limit 100
"""
...
def export_traces(
    ...
    try:
        ...
        # Calculate timestamp filter (with UTC timezone)
    ...
    for trace in traces:
        ...
        if isinstance(resource_attrs, dict):
            ...
            if service_name == "unknown_service":
                issues.append({
                    "trace_id": trace_id,
                    "issue": "unknown_service_metadata",
                    "name": trace_name,
                    "message": "Metadata has 'unknown_service' as service name"
                })
