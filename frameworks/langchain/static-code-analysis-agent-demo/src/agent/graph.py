"""
LangGraph ReAct Agent implementation for static code analysis.
"""
...
def create_agent(config: Optional[Config] = None):
    """Create the LangGraph ReAct agent for static code analysis."""
    if config is None:
        config = Config()
        config.validate()
...
async def run_agent(
    ...
    """Run the ReAct agent for code analysis."""
